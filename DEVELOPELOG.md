...
## 2026-05-01 - v0.6.8 進階基本面與宏觀指標升級

### 設計思路
為了提升 AI 圓桌顧問的判斷精準度，減少因過於關注短線技術指標而產生的「見樹不見林」問題，我們引入了宏觀尾部風險與估值深度指標：

1. **基本面防護網**：引入 PEG 與 Forward P/E，並讓 Peter Lynch 顧問執行「PEG > 2 堅決拒買」的紀律。
2. **宏觀尾部風險**：引入 SKEW 指數，為 Taleb 顧問提供判斷「市場是否在為黑天鵝定價」的量化依據。
3. **鏈上基本面**：加密幣加入 TVL 指標，幫助 Paul Graham 顧問過濾無真實資金沉澱的空氣幣/資金盤。
