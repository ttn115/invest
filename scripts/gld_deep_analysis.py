...
def run():
    ...
    gld_1d = gld_1d.dropna(subset=["Close"])
    close_1d = gld_1d["Close"]
    vol_1d = gld_1d["Volume"]
    close_1wk = gld_1wk["Close"].dropna()
    close_1h = gld_1h["Close"].dropna() if not gld_1h.empty else pd.Series(dtype=float)

    # 指標基準＝最後一根已完成日線；現價＝最新成交價（含盤前/盤後）
    daily_px   = float(close_1d.iloc[-1])
    daily_date = close_1d.index[-1].strftime("%Y-%m-%d")
    q = get_fresh_quote("GLD", daily_close=daily_px, daily_date=daily_date)
    ...
    for ln in freshness_banner("GLD", q):
        print(f"  {ln}")
    print(f"  GLD 現價：${price:.2f}（最新成交）  |  日線收盤 ${daily_px:.2f}（{daily_date}）")
    ...
    rinf_df = yf.Ticker("RINF").history(period="6mo", interval="1d")
